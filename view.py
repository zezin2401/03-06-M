    ...
    try:
        ...
        for filme in filmes:
            filmes_lista.append({
                ...
                'banner': f"static/uploads/cadastro_filme/Banner - {filme[0]}.jpeg",
                'link': filme[8]
            })
...
@app.route('/promocao/<int:id_promo>', methods=['GET'])
def buscar_promocao(id_promo):
    try:
        cur = con.cursor()
        cur.execute("""
            SELECT id_promo, titulopromo, duracaopromo, generopromo, descricaopromo 
            FROM promocao WHERE id_promo = ?
        """, (id_promo,))
        ...
        promocao = {
            'id_promo': resultado[0],
            'titulo': resultado[1],
            'duracaopromo': resultado[2],
            'generopromo': resultado[3],
            'descricaopromo': resultado[4],
            'capapromo': f"static/uploads/promocao/{resultado[0]}.jpeg"
        }

        return jsonify({'mensagem': 'Promoção encontrada', 'promocao': promocao}), 200
...
@app.route('/pagamento/<int:id_reserva>', methods=['GET'])
def mostrar_pagamento(id_reserva):
    cursor = con.cursor()

    # Consulta os dados principais da reserva
    cursor.execute("""
        SELECT 
            u.NOME,
            cf.TITULO, 
            ...
            r.VALOR_TOTAL
        FROM RESERVA r
        JOIN CADASTRO u ON u.ID_CADASTRO = r.ID_CADASTRO
        JOIN SESSAO s ON s.ID_SESSAO = r.ID_SESSAO
        JOIN CADASTRO_FILME cf ON cf.ID_CADASTROF = s.ID_CADASTROF
    ...
    cursor.execute("""
        ...
        FROM RESERVA_ASSENTOS ra
        JOIN ASSENTOS a ON a.ID_ASSENTO = ra.ID_ASSENTO
        WHERE ra.ID_RESERVA = ?
    """, (id_reserva,))
    ...
    return jsonify({
        ...
        "titulo_filme": titulo,
        "nome_sala": nome_sala,
        "data_sessao": data_sessao.strftime('%Y-%m-%d %H:%M'),
        "assentos": assentos,
...
def total_arrecadado():
    try:
        cur = con.cursor()
        cur.execute("""
            SELECT CAST(COALESCE(SUM(VALOR_TOTAL), 0) AS DOUBLE PRECISION) AS total
            FROM RESERVA
        """)
        resultado = cur.fetchone()
        cur.close()

        total = float(resultado[0]) if resultado and resultado[0] is not None else 0.0

        return jsonify({
            "mensagem": "Total arrecadado em bilheteria",
...
def atualizar_avaliacao(id_avaliacao):
    ...
    try:
        ...
        return jsonify({'error': str(e)}), 500
@app.route('/media_avaliacoes/<int:id_cadastrof>', methods=['GET'])
def media_avaliacoes_filme(id_cadastrof):
    try:
        cur = con.cursor()

        # Buscar todas as notas do filme
        cur.execute("""
            SELECT NOTA
            FROM AVALIACAO
            WHERE ID_CADASTROF = ?
        ...
        media_nota = round(sum(notas) / total_avaliacoes, 2) if total_avaliacoes > 0 else None

        cur.close()

        resultado = {
            'id_cadastrof': id_cadastrof,
